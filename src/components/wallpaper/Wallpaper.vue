<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>已审核壁纸</el-breadcrumb-item>
    </el-breadcrumb>
        ...
        <el-col :span="7">
          ...
          >
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="findWallpaperByTitleExamine(queryInfo)"
            />
          </el-input>
        </el-col>
        <el-col :span="4">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addWallpaper">确 定</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    async getWallpaperList() {
      const { data: res } = await this.$http.get('/wallpaper/findAllWallpaperExamine')
      if (res.statue !== 200) {
    ...
    //  根据壁纸名称模糊查询
    async findWallpaperByTitleExamine(queryInfo) {
      const { data: res } = await this.$http.get(
        `/wallpaper/findWallpaperByTitleExamine?title=${queryInfo.query}`
      )
      console.log(res)
      if (res.statue !== 200) {
